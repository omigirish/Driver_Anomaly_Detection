...
try:
    print("Welcome: ",+sys.version_info.major,"---",sys.version_info.minor)
    ...
    def restart(self):
        ...
        # Set up the sensors.
            if preview:
                ...
                spawn_point = carla.Transform(carla.Location(x=2.5,z=2.5)) #Relative coordinates of mounting point wrt Car
                sensor = self.world.spawn_actor(cam_bp,spawn_point, attach_to=self.player) # Attach to the car
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        if keys[K_LEFT] or keys[K_a]:
            ...
            print(self._steer_cache,milliseconds)
...
class HUD(object):
    ...
    def write_csv(self,c,v,sec_prev,tprev,sprev,bprev,speedprev):
        ...
        if lapse > 0.5:
            with open('data.csv','a',newline='') as f:
                ...
                thewriter.writerow({"DThrottle" : round(dt,1), "DSteer" : round(ds,1), "Speed" : round(3.6*math.sqrt(v.x**2 + v.y**2 + v.z**2),0), "DBrake" : round(db,1),"Eye_blinks":COUNTER2,"lane_invades":math.ceil(lane_invades),"Dspeed":dspeed })
    ...
    def tick(self, world, clock):
        ...
        if isinstance(c, carla.VehicleControl):
            ...
            # with open('data.csv','a',newline='') as f:
            #     fieldname=["Throttle","Steer","Speed","Brake"]
            #     thewriter=csv.DictWriter(f,fieldnames=fieldname)
            #     # thewriter.writeheader()
            #     thewriter.writerow({"Throttle" : round(c.throttle,1), "Steer" : round(c.steer,1), "Speed" : round(3.6*math.sqrt(v.x**2 + v.y**2 + v.z**2),0), "Brake" : round(c.brake,1)})
                # print({"Throttle" : c.throttle, "Steer" : c.steer, "Speed" : math.sqrt(v.x**2 + v.y**2 + v.z**2), "Brake" : c.brake})
